...
import { CloudIcon, CostIcon, AutomationIcon, AIIcon, WebDevIcon, HostingIcon } from './components/Icons'
...
const ServiceCard = ({ icon: Icon, title, description }: { icon: React.ReactNode; title: string; description: string }) => (
  <div className="bg-white rounded-xl p-6 md:p-8 shadow-md hover:shadow-xl transition-shadow border-t-4 border-blue-500 group">
    ...
    <>
      ...
      <header className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-50 animate-fade-in">
        <div className="max-w-6xl mx-auto px-4 md:px-6 py-4 md:py-6">
          <div className="flex items-center gap-2 md:gap-3 mb-2 animate-slide-in-left">
            <Logo />
            <h1 className="text-2xl md:text-3xl font-bold text-black font-alfa">South Side Tech</h1>
          </div>
          <p className="text-gray-600 text-sm md:text-base animate-slide-in-right animate-delay-100">Tech Solutions for Every Business</p>
        </div>
      </header>
      ...
      <main className="flex-grow">
        {/* Hero Section */}
        <section className="bg-gradient-to-br from-blue-50 to-blue-100 py-16 md:py-24 px-4 md:px-6">
          ...
          <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {[0, 100, 200, 300, 400, 500].map((delay, idx) => (
              ...
              {[0, 100, 200].map((delay, idx) => (
                <ScrollAnimation key={idx} animation="slide-up" delay={delay}>
                  <div className="text-center group hover:transform hover:scale-105 transition-transform duration-300">
                    <div className="bg-blue-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-blue-200 group-hover:shadow-lg transition-all">
                      <span className="text-2xl font-bold text-blue-600">
                        {idx === 0 && <AnimatedCounter target={10} suffix="+" />}
                        {idx === 1 && <AnimatedCounter target={500} suffix="+" />}
                        {idx === 2 && '24/7'}
                      </span>
                    </div>
                    ...
                    <p className="text-gray-700">
                      ...
                      {idx === 1 && 'Trusted by startups, SMBs, and enterprises worldwide'}
                      {idx === 2 && 'Round-the-clock support to keep your business running smoothly'}
                    </p>
                  </div>
                </ScrollAnimation>
              ))}
        ...
        {/* CTA Section */}
        <section className="bg-gradient-to-r from-blue-600 to-blue-800 py-16 md:py-20 px-4 md:px-6">
          <div className="max-w-4xl mx-auto text-center">
            <ScrollAnimation animation="slide-up">
              <h3 className="text-3xl md:text-4xl font-bold text-white mb-6 font-playfair">Ready to Transform Your Business?</h3>
            </ScrollAnimation>
            <ScrollAnimation animation="slide-up" delay={100}>
              <p className="text-blue-100 mb-8 text-lg md:text-xl">Take our quick questionnaire and let's discover how we can help optimize your technology infrastructure</p>
            </ScrollAnimation>
            <ScrollAnimation animation="bounce-in" delay={200}>
              <Link href="/questionnaire" className="inline-block bg-white hover:bg-blue-50 text-blue-600 font-bold py-4 px-8 md:px-10 rounded-lg transition-all duration-300 text-lg shadow-lg hover:shadow-2xl hover:scale-105 transform">
