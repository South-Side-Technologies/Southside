...
export default function Header({ variant = 'public', currentUser, subtitle }: HeaderProps) {
  ...
  return (
    <header className="bg-gray-800 shadow-sm border-b border-gray-700 sticky top-0 z-50 animate-fade-in">
      <div className="max-w-6xl mx-auto px-4 md:px-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-3 py-4 md:py-6">
          <Link href="/" className="flex items-center gap-2 md:gap-3 animate-slide-in-left hover:opacity-80 transition-opacity justify-center md:justify-start">
            <Logo />
            <h1 className="text-2xl md:text-3xl font-bold text-white font-alfa">South Side Tech</h1>
          </Link>

          {variant === 'public' && !currentUser && (
            <p className="text-gray-400 text-sm md:text-base animate-slide-in-right animate-delay-100 text-center md:text-right">
          ...
          {variant === 'authenticated' && currentUser && (
            <div className="flex items-center gap-4">
              ...
              >
                ...
                <span className="text-gray-300 font-medium hidden md:block">{getUserDisplayName(currentUser)}</span>
        ...
        {variant === 'public' && (
          <nav className="flex flex-wrap gap-2 pb-4 border-t border-gray-700 justify-center md:justify-start">
            <Link href="/customer-login" className="px-4 py-2 text-gray-300 font-semibold border-b-2 border-transparent hover:border-b-2 hover:border-red-700 hover:text-red-700 transition-colors duration-300 text-sm md:text-base">
              Customer Login
            </Link>
            <Link href="/customer-login" className="px-4 py-2 text-gray-300 font-semibold border-b-2 border-transparent hover:border-b-2 hover:border-red-700 hover:text-red-700 transition-colors duration-300 text-sm md:text-base">
              Start Consultation
            </Link>
            <Link href="/#chatbot" className="px-4 py-2 text-gray-300 font-semibold border-b-2 border-transparent hover:border-b-2 hover:border-red-700 hover:text-red-700 transition-colors duration-300 text-sm md:text-base">
              Chatbot
            </Link>
          </nav>
        )}
